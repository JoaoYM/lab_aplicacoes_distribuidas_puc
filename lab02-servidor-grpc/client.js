...
const GrpcLoadBalancer = require('./utils/grpcLoadBalancer');
...
class GrpcClient {
    constructor(serverAddress = 'localhost:50051') {
        if (Array.isArray(serverAddresses)) {
            ...
            this.loadBalancer.initializeClients();
            this.useLoadBalancing = true;
        } else {
            // Modo tradicional com único servidor
    ...
    async createTask(taskData) {
        if (this.useLoadBalancing) {
            return await this.loadBalancer.execute('CreateTask', {
